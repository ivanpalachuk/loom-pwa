...
export default function WaterAnalysisPage() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-loom-10 to-blue-50">
            {/* Header */}
            ...
            <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
                ...
                <div className="relative rounded-xl overflow-hidden shadow-lg bg-white">
                    <img src={imageData} alt="Water test strip" className="w-full max-h-64 object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent opacity-30" />
                </div>

                {analyzing ? (
                    /* Loading State */
                    <div className="flex flex-col items-center justify-center py-12 space-y-4 bg-white rounded-xl shadow-md">
                        ...
                        <p className="text-lg font-medium text-gray-600">Analizando tira reactiva...</p>
                        <p className="text-sm text-gray-500">Detectando dureza y pH</p>
                    </div>
                ) : analysis && (
                    <>
                        {/* Quality Badge */}
                        <div className="flex items-center justify-center">
                            <div
                                className="px-8 py-4 rounded-full text-white font-bold text-xl shadow-lg transform hover:scale-105 transition-transform"
                                style={{ backgroundColor: getQualityColor(analysis.quality) }}
                            >
                                Calidad: {getQualityLabel(analysis.quality)}
                            </div>
                        </div>
                        ...
                        {/* Recommendations */}
                        <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-6 rounded-xl shadow-md">
                            ...
                            <ul className="space-y-3">
                                {analysis.recommendations.map((rec, index) => (
                                    <li
                                        key={index}
                                        className="flex items-start gap-3 bg-white p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow"
                                    >
                                        <span className="text-lg flex-shrink-0 mt-0.5">{rec.split(':')[0]}</span>
                                        <span className="text-gray-700">{rec.split(':').slice(1).join(':').trim()}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div className="flex gap-3 pb-4">
                            <button
                                onClick={handleBack}
                                className="flex-1 bg-gradient-to-r from-loom to-loom-70 text-white py-3 px-6 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all"
                            >
                                Volver al Inicio
                            </button>
                        </div>
